    ...
    const getWeatherIcon = (description) => {
        ...
        return (
            <div className="weather-info">
                ...
                <p>Máx.: {forecast?.[0]?.max}° / Mín.: {forecast?.[0]?.min}° </p>
                ...
                <ul className="forecast-info">
                    {forecast?.slice(2, 5).map((day, index) => {
                        ...
                        return (
                            <li key={index}>
                                <strong>{date ? date.toLocaleDateString() : 'Data inválida'}</strong>: 
                                ↑ {day.max}° / ↓ {day.min}°<br /> 
                                Clima: {day.description}<br /> 
                                {weatherImageSrc && (
                                    <img
                                        src={weatherImageSrc} 
                                ...
                                <br />
                                Chance de Chuva: {day.rain_probability ? day.rain_probability : '0'}% 
                            </li>
                        );
                    })}
                </ul>
