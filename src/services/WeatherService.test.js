...
import { getWeatherData } from './WeatherService'; // Substitua 'seuArquivo' pelo nome do arquivo onde a função está
...
describe('Testes da função getWeatherData', () => {
  ...
  it('deve lançar um erro se a cidade não for encontrada', async () => {
    ...
    await expect(getWeatherData(city)).rejects.toThrow('Cidade não encontrada.');
  });
  ...
  it('deve lançar um erro se as informações do clima não forem encontradas', async () => {
    ...
    await expect(getWeatherData(city)).rejects.toThrow('Não foi possível obter as informações do tempo.');
  });
});